...
use App\User;
use App\Models\TfWaterUsedImport;
...
class WaterController extends Controller {
    ...
    public function anyData() {    
        ...
        // dd(123);
        $TfWaterUsedImport = DB::table('tf_water_used_import')
            ->select(['*'])
            ->where('id', '<>' , 0)
        ...
        return Datatables::of($TfWaterUsedImport)
                ->addColumn('action', function ($TfWaterUsedImport) {
                    return '<a href="flat/'. $TfWaterUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Chọn</a>';                    
                })
                ->addColumn('water', function ($TfWaterUsedImport) {
                    return '<a href="flat/water/'. $TfWaterUsedImport->id . '" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('water', function ($TfWaterUsedImport) {
                    return '<a href="flat/water/'. $TfWaterUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ...
                ->addColumn('parking', function ($TfWaterUsedImport) {
                    return '<a href="flat/parking/'. $TfWaterUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->addColumn('service', function ($TfWaterUsedImport) {
                    return '<a href="flat/service/'. $TfWaterUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->make(true);        
    }        
    ...
    {
        return Excel::create('import_water' . $request->year . $request->month, function($excel) use ($request) {
        ...
        if(Input::hasFile('import_file')){
            ...
            if(!empty($data) && $data->count()){
                ...
                if(!empty($insert)){
                    ...
                    $old_data = TfWaterUsedImport::where('tenement_id', $tenement_id)
                        ->where('token', Session::token())
                        ->delete();

                    DB::table('tf_water_used_import')->insert($insert);

                    DB::statement("
                        CALL proc_getOldIndex(
                            '" . $request->_token . "', 
                            '" . $tenement_id ."' ,
                            'water'
                            )
                    ...
                    return redirect('import/importWater');
                }
            }
        }
    }
