        ...
        return Excel::create('import_flat' . $request->year . $request->month, function($excel) use ($request) {
                $excel->sheet('data', function($sheet) use ($request)
                {
                    ...
                    for($i=0;$i<count($request->counter);$i++){
                        ...
                        $blockName = 'block_name'.$step;
                        //$blockNum = 'flat_block_num'.$step;
                        $floorFrom = 'floor_num_from'.$step;
                        $floorTo = 'floor_num_to'.$step;
        ...
        if(Input::hasFile('import_file')){
            ...
            if(!empty($data) && $data->count()){
                ...
                foreach ($data as $key => $value) {
                    ...
                    $flat_num = '';
                    if (strlen($value->can_ho_so) > 2){
                        $flat_num = $request->input('flat_num');
                    }
                    else{
                        $flat_num = str_pad($value->can_ho_so,2,"0", STR_PAD_LEFT)
                        ;
                    }
                    ...
                    $insert[] = [
                        ...
                        'flat_code' =>  $value->ten_khu . 
                                        $value->khu_so . "-" .
                                        $value->tang_so . $value->tang_so_phu ."-" .
                                        $value->can_ho_so,

                        'block_name' => $value->ten_khu,
                        'floor_num' => $value->tang_so,
                        'floor_name' => $value->tang_so_phu,
                        'flat_num' => $flat_num,

                        'address' => $address,
                        'name' => $value->chu_ho,
                        'contact' => $value->dia_chi_lien_lac,
                        'contract_no' => $value->so_hop_dong,
                        'email' => $value->email,
