...
use App\User;
use App\Models\TfGasUsedImport;
...
class GasController extends Controller {
    ...
    public function anyData() {    
        ...
        // dd(123);
        $TfGasUsedImport = DB::table('tf_gas_used_import')
            ->select(['*'])
            ->where('id', '<>' , 0)
        ...
        return Datatables::of($TfGasUsedImport)
                ->addColumn('action', function ($TfGasUsedImport) {
                    return '<a href="flat/'. $TfGasUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Chọn</a>';                    
                })
                ->addColumn('gas', function ($TfGasUsedImport) {
                    return '<a href="flat/gas/'. $TfGasUsedImport->id . '" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('water', function ($TfGasUsedImport) {
                    return '<a href="flat/water/'. $TfGasUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('gas', function ($TfGasUsedImport) {
                    return '<a href="flat/gas/'. $TfGasUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('parking', function ($TfGasUsedImport) {
                    return '<a href="flat/parking/'. $TfGasUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->addColumn('service', function ($TfGasUsedImport) {
                    return '<a href="flat/service/'. $TfGasUsedImport->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->make(true);        
    }        
    ...
    {
        return Excel::create('import_gas' . $request->year . $request->month, function($excel) use ($request) {
        ...
        if(Input::hasFile('import_file')){
            ...
            if(!empty($data) && $data->count()){
                ...
                if(!empty($insert)){
                    ...
                    $old_data = TfGasUsedImport::where('tenement_id', $tenement_id)
                        ->where('token', Session::token())
                        ->delete();

                    DB::table('tf_gas_used_import')->insert($insert);

                    return redirect('import/importGas');
                }
            }
        }
    }
