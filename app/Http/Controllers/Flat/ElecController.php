...
class ElecController extends Controller {
    ...
    public function index(Request $request, $id) {
        ...
        $TfElecUsed = TfElecUsed::where('flat_id',$id)
            ->where('activation',1)
            ->orderBy('year_month', 'asc')->first();
            // dd($TfElecUsed);

        //dd($Tenement[0]);
        return View('flat.flatelec', [ 'TfElecUsed'=>$id, 'year_month'=>$year_month]);
    }
    ...
    public function anyData($id) {        
        ...
        // dd(123);
        $TfElecUsed = DB::table('tf_elec_used')
            ->select(['*'])
            ->where('activation', 1)
            ->where('id', '<>' , 0)
            ->where('flat_id', '=', $id)
        ...
        return Datatables::of($TfElecUsed)
                ->addColumn('action', function ($TfElecUsed) {
                    return '<button type="button" class="btn btn-primary btn-details" value="'. $TfElecUsed->id .'" >Hủy</button>';
                })
                ->addColumn('elec', function ($TfElecUsed) {
                    return '<a href="flat/elec/'. $TfElecUsed->id . '" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('water', function ($TfElecUsed) {
                    return '<a href="flat/water/'. $TfElecUsed->id .'" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('gas', function ($TfElecUsed) {
                    return '<a href="flat/gas/'. $TfElecUsed->id .'" class="btn btn-xs btn-primary" target="_blank">Chỉ số tháng</a>';                    
                })
                ->addColumn('parking', function ($TfElecUsed) {
                    return '<a href="flat/parking/'. $TfElecUsed->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->addColumn('service', function ($TfElecUsed) {
                    return '<a href="flat/service/'. $TfElecUsed->id .'" class="btn btn-xs btn-primary" target="_blank">Xe tháng</a>';                    
                })
                ->make(true);        
    }        
    ...
    public function store(Request $request)
    {
        ...
        if($v->fails()){
            return Redirect::to('flat/elec/' . $request->input('id'))
        ...
        $existedYM = TfElecUsed::where('year_month', $year_month)
                        ->where('activation',1)
    ...
    public function destroy($id)
    {
        ...
        DB::beginTransaction();
        try {
            $TfElecUsedExits = DB::table("tenement_flats")
            ->join('tf_elec_used', 'tenement_flats.id', '=', 'tf_elec_used.flat_id')
            ->select('tenement_flats.*')
            ->where('tenement_flats.activation', '=', 1)
            ->where('tenement_flats.tenement_id', '=', $tenement_id)
            ->where('tf_elec_used.id', '=', $id)->count();

            if($TfElecUsedExits == 0){
                return Response::view('errors.404', array(), 404);
            }  

            $Used = TfElecUsed::find($id);
    ...
    public function allindex($year_month) {
        ...
        if ($year_month == 'detail')
            $year_month = date("Ym");
        //dd($Tenement[0]);
        return View('flat.allflatelec', [ 'year_month'=>$year_month]);
    }
    ...
    public function allFlatData($year_month) {
        ...
        $monthlyFeeFlat = DB::table('tenement_flats as a')
            ->leftjoin('tf_elec_used as b', function($join) use ($year_month)
            {
    ...
    public function exex_change(Request $request)
    {
        ...
        try {
            ...
            $TfElecUsedExits = DB::table("tenement_flats")
                ...
                ->where('tenement_flats.activation', '=', 1)
                ->where('tenement_flats.tenement_id', '=', $tenement_id)
                ->where('tf_elec_used.flat_id', '=', $request->flat_id)
                ->where('tf_elec_used.year_month', '=', $year_month)->get();

            if (isset($TfElecUsedExits[0])){
                $Used = TfElecUsed::find($TfElecUsedExits[0]->id);
                $Used->activation = 0;
                $Used->updated_by = Auth::user()->id;
                $Used->updated_at = date("Y-m-d H:i:s");                
                $Used->save();
            ...
            DB::commit();
            return redirect('flat/all/elec/' . $year_month)->with('tenementElec-alert-success','Đã thực hiện Cập nhật chỉ số thành công !');
        } catch (\Exception $e) {
            DB::rollback();
