...
<script>
    ...
    $(function(){
        ...
        $("#tenementElecRefresh").manualRefresh('frmTenementElec');
        ...
        @role('admin|manager|moderator')
            ...
            $("#tenementElecDelete").click(function(){
        ...
        $(button_my_button).click(function(){
            ...
            var data="<tr class='new'><td class='text-center'><input type='checkbox' class='case'/></td>";
                ...
                data+="<td><input class='form-control text-right commas price' name='index_from"+count+"' id='index_from_"+count+"' class='form-control service'></td>";
                data+="<td><input class='form-control text-right commas price' name='price"+count+"' id='price_"+count+"' class='form-control service'></td>";
                data+="<td><input class='form-control text-right commas price' name='vat"+count+"' id='vat_"+count+"' class='form-control service'></td>";
                data+="<td><input class='form-control text-right commas price' name='other_fee01"+count+"' id='other_fee01_"+count+"' class='form-control service'></td>";
                data+="<td><input class='form-control text-right commas price' name='other_fee02"+count+"' id='other_fee02_"+count+"' class='form-control service'></td>";
...
<div class="table-responsive">
    <div class="flash-message">
        ...
        @if(Session::has('tenementElec-alert-' . $msg))
    ...
    <form id='frmTenementElec' action="{!! route('TenementElec.update') !!}" method="POST" role="form" method="post">
        <table class="table table-condensed xs table-striped">
            <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">                
            <input id="tenement_elec_id" name='tenement_elec_id' type="hidden" value="{!! $tenementElec->id !!}">
            <tbody>
                <tr>
                    <td>Biểu phí điện sử dụng<em style='color:red'>(*)</em> </td>
                    <td><input type="text" value="{!! $tenementElec->elec_type !!}" id="elec_type" name="elec_type" size="50%"></td>
                    <td>Ghi chú</td>
                    <td><input type="text" value="{!! $tenementElec->comment !!}" id="comment" name="comment" size="50%"></td>
                </tr>
            </tbody>
        </table>
            ...
            <tbody>
                ...
                ?>
                            <tr class='old'>
                                ...
                                <td>
                                    <input class='form-control unitprice price' type='text' data-type='charge' id='name_{!! $step !!}' name='name{!! $step !!}' value='{!! $value->name or 0 !!}' />
                                </td>
                                <td>
                                ...
                                <td>
                                    <input class='form-control text-right text-right commas price' type='text' data-type='charge' id='vat_{!! $step !!}' name='vat{!! $step !!}' value='{!! $value->vat or 0 !!}'/>
                                </td>

                                <td>
                                    <input class='form-control text-right commas price' type='text' data-type='charge' id='other_fee01_{!! $step !!}' name='other_fee01{!! $step !!}' value='{!! $value->other_fee01 !!}'/>
                                </td>
    ...
    @role('admin|manager|moderator')
        <div>
            ...
            <button id='tenementElecSubmit' type="button" class="btn btn-primary">Lưu</button>            
            <button id='tenementElecDelete' type="button" class="btn btn-default">Xóa</button> &nbsp;
            <a href="{!! route('TenementElec') !!} " class="btn btn-info">Trở về màn hình trước</a>

            <form id='frmTenElec' action="{!! route('TenementElec.destroy') !!}" method="POST" role="form" method="post">
                <input type="hidden" name="_token" value="<?php echo csrf_token(); ?>">                
                <input id="tenement_elec_id" name='tenement_elec_id' type="hidden" value="{!! $tenementElec->id !!}">
            </form>
        </div>
