...
<script>
	...
	$(function () {
		...
		var equipmentsList = {!! json_encode($tenement_equipments) !!};
		...
		$('#calendar').fullCalendar({
		  ...
		  //       $.ajax({
		    ...
		    eventClick: function(event, jsEvent, view) {
	            $("#name").val(event.title);
	            $("#report_date").val(event.report_date);
	            $("#id").val(event.id);

				$("#report_start_time").val(event.report_start_time);
				$("#report_end_time").val(event.report_end_time);
				$("#report_for").val(event.report_for);
				$("#report_description").val(event.report_description);
				$("#report_company_execute").val(event.report_company_execute);
				$("#note").val(event.note);
  ...
  <div class="container">
    <div class="row">
      <div class="col-sm-12">
        ...
        <h2>Kế Hoạch Bảo Trì Thiết Bị - Máy Móc - Vật Tư</h2>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="col-lg-12">
		...
		<div id='wrap'>
			<div id='external-events'>
				@foreach($tenement_equipments as $item)
		            <div class='fc-event' id="{!! $item->name !!}">                    
                        <a href="schedule/{!! $item->id !!}" style="color: white;">{!! $item->name !!}</a>
                        ...
                        @foreach($tenement_equipments as $item)
                            <tr>                    
                                <td class="text-center">
                                	<a href="schedule/{!! $item->id !!}">{!! $item->name !!}</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="reportmodAlLabel">Báo Cáo Thực Hiện Kế Hoạch Bảo Trì</h4>
				</div>
				<form id='frmSubmit' action="{!! route('EquipMainte.report') !!}" method="POST" role="form">
					...
					<table class="table table-striped table-bordered table-hover table-condensed">
						...
						<tbody>
		                    <tr>
		                        ...
		                        <td><input type="text" class='date-picker' value='{!! date('d/m/Y') id="report_date" name="report_date" size="30%"></td>
		                 	</tr>
		             	    <tr>
		                        <td>Thời gian<em style='color:red'>(*)</em> </td>
		                        <td>
			                        <div class="input-group bootstrap-timepicker timepicker">
				                        <input type="text" class='timepicker' value="" id="report_start_time" name="report_start_time" size="10%"> ~ 
				                        <input type="text" class='timepicker' value="" id="report_end_time" name="report_end_time" size="10%">
						        	</div>
							    </td>
		                 	</tr>
		             	    <tr>
		                        <td>Người phụ trách<em style='color:red'>(*)</em> </td>
		                        <td><input type="text" value="" id="report_for" name="report_for" size="30%"></td>
		                 	</tr>
		             	    <tr>
		                        <td>Mô tả<em style='color:red'>(*)</em> </td>
		                        <td><input type="text" value="" id="report_description" name="report_description" size="30%"></td>
		                 	</tr>
		             	    <tr>
		                        <td>Công ty thực hiện<em style='color:red'>(*)</em> </td>
		                        <td><input type="text" value="" id="report_company_execute" name="report_company_execute" size="30%"></td>
		                 	</tr>
		                 	<tr>
		                 		<td>Hạng mục bảo trì
		                 		</td>
							...
							<tr>
		             	    ...
		             	    <tr>
		                        ...
		                        <td><input type="text" value="" id="note" name="note" size="30%"></td>
		                 	</tr>
	             		</tbody>
	             	</table>
					</div>
