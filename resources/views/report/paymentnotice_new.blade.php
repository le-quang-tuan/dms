...
@section('content')
    ...
    <div id="aaa">
        ...
        $flat_info = $paymentnotice;
        $total_money = $flat_info->total_money;
        $total_money_read = $flat_info->total_money_read;
        $total_money_readEn = $flat_info->total_money_readEn;
        $payment = $flat_info;
        $elec = $paymentnotice[1];
        $water = $paymentnotice[2];
        $gas = $paymentnotice[3];
        $parking = $paymentnotice[4];
        $service = $paymentnotice[5];
        $dept = $paymentnotice[6];
        ...
        <div class="header">
            ...
            if (date('Ym') < $payment->year_month){
                $bill_date = '01/' . substr($payment->year_month, -2) . '/' . substr($payment->year_month, 0, -2);
            } 
            echo $bill_date;
            ?>
            </h3>
        ...
        <div>
            <div>
                ...
                <div class="col-xs-11">
                    <i>We are pleased to inform about the fee in <?php 
                    $bill_date = strtotime(substr($payment->year_month, -2) . '/01/' . substr($payment->year_month, 0, -2));
                    $bill_date = date('M, Y', $bill_date);
                    echo $bill_date;
                    ?></i>
                </div>
        ...
        <?php
            $manager_fee = $payment->manager_fee - $payment->manager_fee_paid;
         ?>
        @if ($manager_fee > 0)
        <?php  
            $number += 1;
                    ...
                    <tbody>
                        ...
                        <tr>                    
                            <td class="text-center">
                                {!! 
                                '01/' . substr($payment->year_month, 4, 2) . '/' . substr($payment->year_month, 0,4) . '~'. date("t/m/Y", strtotime(substr($payment->year_month, 0, 4) . '-' . substr($payment->year_month, 4,2))) 
                                !!}
                            </td>
                            <td class="text-center">
                ...
                <tbody>
                    ...
                    @if (isset($water_dt->total_hd))    
                        <tr>
                            ...
                            <td class="text-right">{!! number_format($water_dt->total) !!}</td>
                             <!-- . $water_dt->other_fee01_money_hd . $water_dt->vat_money_hd . $water_dt->other_fee02_money_hd -->
                        </tr>
                    @endif
        ...
        @if (isset($parking[0]->total_count) && $payment->parking_fee_paid < $payment->parking_fee)
        <?php
            $number += 1;
            $stringNumber .= " + " . $number;
        ?>
        <div>
            <div class="col-xs-11">
                ...
                @if ($payment->parking_fee_paid >0)
                    Phí đã trả: {!! number_format($payment->parking_fee_paid) !!}
                @endif
            </div>
        </div>
        <div>
        ...
        <div>
            <div class="col-xs-11">
                <b>&nbsp;&nbsp;&nbsp;&nbsp;  {!! $number !!}. Hoàn tiền phí quản lý:&nbsp;</b>
                    {!! number_format($service[0]->total_money*(-1)) !!}(VND)
            </div>
        </div>
        <!-- <div>
            <div>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <tr>                    
                                ...
                                <td class="text-center">
                                    {!! number_format($service[0]->mount) !!}
                                </td>
                                <td class="text-center">
                                    {!! number_format($service[0]->total_money) !!}
                                </td>
                            </tr>
                    </tbody>
                </table>
        ...
        <div class="row voffset1">
            ...
            <div class="col-xs-11">
                <div class="col-xs-12 text-left" style="font-size: 10px">
                    ...
                    Ví dụ: Căn khách {!! $flat_info->name !!} số: {!! $flat_info->address !!}. <br>
                    Nội dung thanh toán ghi như sau: Thanh toán phí quản lý, tiền nước, giữ xe căn hộ {!! $flat_info->address !!} kỳ <?php echo substr($payment->year_month, -2) . '/' . substr($payment->year_month, 0, -2);?>.<br><br>
