...
<script>
    ...
    $(function () {
        ...
        $('#users-table').DataTable({
            ...
            serverSide: false,
            deferLoading: 0,
            ajax: '{!! url("/import/importService/anyData") !!}',
            columns: [
                {
                    data: null, 
        ...
        $('#tenementFlatServiceSubmit').on('click', function(){
            // alert("aaaaa");
        ...
        $("#tenementFlatServiceSubmit").manualSubmit('frmtenementFlatService');

        $("#importSubmit").click(function(){
            bootbox.confirm("Thực hiện thêm mới phí khác cho các căn hộ?", function(result) {
                if(result){
...
<div class="table-responsive">
    ...
    <div class="col-lg-12">
        {{ Form::open(['route' => ['ImportService.download'],  'method' => 'POST', 'id'=>'frmtenementFlatService']) }}
            {!! csrf_field() !!}
            ...
            <table class="table table-striped table-bordered table-hover table-condensed">
                <tbody>
                    <tr class="info" style="color: black">
                        <td>Dịch vụ khác để xuất Phiếu thu tháng<em style='color:red'>(*)</em> </td>
                        <td colspan="4">
                        ...
                        <td>Dịch vụ<em style='color:red'>(*)</em></td>
                        <td colspan="5"><input type="text" value="{!! old('name') !!}" id="name" name="name" size="50%"></td>
                    </tr>
                    <tr class="info" style="color: black">
                        <td>Dịch vụ khác từ ngày<em style='color:red'>(*)</em></td>
                        <td colspan="4"><input class="date-picker" type="text" value="{!! old('date_from') !!}" id="date_from" name="date_from" size="10%">~<input class="date-picker" type="text" value="{!! old('date_to') !!}" id="date_to" name="date_to" size="10%"></td>
                        <td>Số lượng<em style='color:red'>(*)</em></td>
                        <td><input type="text" value="{!! old('mount') !!}" id="mount" name="mount" size="10%"></td>
                        <td>Đơn vị<em style='color:red'>(*)</em></td>
                        <td><input type="text" value="{!! old('unit') !!}" id="unit" name="unit" size="10%"></td>
                        <td>Đơn giá<em style='color:red'>(*)</em></td>
                        <td><input type="text" value="{!! old('price') !!}" id="price" name="price" size="10%"></td>
                    </tr>
                    <tr class="info" style="color: black">
                        <td>Ghi chú</td>
                        <td colspan="10">
                            <input type="text" name="comment" id="comment" name="comment" size="80%" value="{!! old('comment') !!}">
                        </td>
                        
                    </tr>
                    <tr>
                        <td colspan="11">
                            <button id='tenementFlatServiceSubmit' type="button" class="btn btn-primary">Tạo File Mẫu Cho Tất Cả Căn Hộ</button>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    <div class="col-lg-12">
        <form style="margin-top: 15px;" action="{{ URL::to('import/importService/store') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
    ...
    <div class="col-lg-12">
        ...
        <form id='frmImportSubmit' style="margin-top: 15px;" action="{{ URL::to('import/importService/save') }}" class="form-horizontal" method="post" enctype="multipart/form-data">
            {!! csrf_field() !!}
        </form>
        <button id='importSubmit' class="btn btn-primary">Thực hiện lưu dữ liệu</button>
    </div>
    <div class="col-lg-12">
        <h3>Chỉ số service được import từ các căn hộ</h3>
        <table class="table table-bordered hover" id="users-table">
            <thead>
                <tr>
                    ...
                    <th class="info">Dịch vụ khác</th>
                    <th class="info">Số lượng</th>
                    <th class="info">Đơn vị</th>
                    <th class="info">Giá tiền</th>
