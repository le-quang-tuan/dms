            ...
            bootbox.confirm(confirmTable, function (result) {
                ...
                if (result == true){
                    ...
                    success:function(data){
                        table.ajax.url('{!! url("monthlyfee/paiddetail/{$id}/anyData") !!}').load();      
                    }});
                }
            });
        });
...
<div class="table-responsive">
    <div class="col-lg-12">
            ...
            <table class="table table-striped table-bordered table-hover table-condensed">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="col-md-1">
                            {!! substr($tf_paid_hd->year_month, 4,2) !!}
                            /{!! substr($tf_paid_hd->year_month, 0,4) !!}
                        </td>
                        <td>Nội dung thu<em style='color:red'>(*)</em></td>
                        <td colspan="3">{!! $tf_paid_hd->comment !!}</td>
                    </tr>
                    <tr>
                        <td>Ngày nhận<em style='color:red'>(*)</em></td>
                        <td>
                            {!! substr($tf_paid_hd->receive_date, 6,2) !!}
                            /{!! substr($tf_paid_hd->receive_date, 4,2) !!}
                            /{!! substr($tf_paid_hd->receive_date, 0,4) !!}
                        </td>
                        ...
                        <td>
                            {!! $tf_paid_hd->receive_from !!}
                        </td>
                    </tr>
                    
                    <tr>
                        <td>Số tiền<em style='color:red'>(*)</em></td>
                        <td>{!! number_format($tf_paid_hd->money) !!}</td>
        ...
        <table class="table">
            <a href="../../report/paidbill/{{ $tf_paid_hd->id }}/2" class="btn btn-xs btn-primary" target="_blank">Xem Phiếu Thu</a> &nbsp;

            <a href="../../report/paidbill/{{ $tf_paid_hd->id }}/1" class="btn btn-xs btn-primary" target="_blank">Download Phiếu Thu</a> &nbsp;

            <a href="../../monthlyfee/paidpayment/{{ $tf_paid_hd->id }}" class="btn btn-xs btn-danger">Chỉnh sửa lại Phiếu Thu</a>
        </table>
